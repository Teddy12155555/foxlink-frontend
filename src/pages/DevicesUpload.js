...
export default function DevicesUpload({token, ...rest}) {
    ...
    const [devideId, setId] = useState([]);
    const [ifsetid, setIfsetid] = useState(false);
    ...
    const setIdfunc = () => {
        ...
        for(let i=1;i<excelData[keys[0]].data.length;i++) {
            excelData[keys[0]].data[i][0] = devideId[i-1];
        }
    ...
    return(
        <ThemeProvider theme={darkTheme}>
            <Card >
            <CardHeader title="車間 Layout 座標表上傳" />
